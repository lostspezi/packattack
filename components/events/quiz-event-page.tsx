  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6 space-y-6">
      ...
      {phase === "join" && (
        <>
          <div className="rounded-2xl border border-border bg-surface p-8 text-center">
            <Zap className="mx-auto mb-4 h-14 w-14 text-pa-green" />
            <h1 className="text-3xl font-bold text-text-primary">{title}</h1>
            <p className="mt-3 text-lg text-text-secondary">
              Teste dein Nerd-Wissen! 15 zufällige Fragen aus der
              Welt der Popkultur.
            </p>
            ...
            {/* Requirements */}
            <div className="mt-6 rounded-xl border border-border bg-surface-alt p-4 text-left">
              <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
                <Shield className="h-4 w-4 text-pa-green" />
                Teilnahmevoraussetzungen
              </div>
              <ul className="mt-2 space-y-1.5 text-sm text-text-secondary">
                <li className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-pa-green" />
                  Du benötigst das{" "}
                  <strong>Beta-Tester Badge</strong>, um teilzunehmen.
                </li>
                <li className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-pa-green" />
                  15 zufällige Fragen aus einem Pool von über 100 Fragen
                </li>
                <li className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-pa-green" />
                  Die 3 besten Spieler gewinnen Preise!
                </li>
                <li className="flex items-center gap-2">
            ...
            {countdown && (
              <div className="mt-6">
                ...
                <p className="mt-1 font-mono text-3xl font-bold text-pa-green">
                  {countdown}
                </p>
              </div>
            )}
            ...
            <button
              onClick={handleJoin}
              disabled={joining}
              className="mt-8 rounded-xl bg-pa-green px-8 py-3.5 text-base font-bold text-black transition-all hover:bg-pa-green/80 hover:scale-105 disabled:opacity-50 disabled:hover:scale-100"
            >
              {joining ? (
      ...
      {phase === "waitlist" && (
        <>
          <div className="rounded-2xl border border-border bg-surface p-8 text-center">
            <CheckCircle2 className="mx-auto mb-4 h-14 w-14 text-pa-green" />
            ...
            {event?.status !== "draft" && countdown && (
              <div className="mt-6">
                ...
                <p className="mt-1 font-mono text-4xl font-bold text-pa-green">
                  {countdown}
                </p>
              </div>
            )}
            ...
            {isAdmin && (
              <button
                onClick={handleStart}
                disabled={starting}
                className="mt-4 rounded-lg border border-yellow-500/30 bg-yellow-500/10 px-6 py-2.5 text-sm font-semibold text-yellow-400 transition-colors hover:bg-yellow-500/20 disabled:opacity-50"
              >
                {starting ? (
      ...
      {phase === "ready" && (
        <>
          <div className="rounded-2xl border border-pa-green/30 bg-surface p-8 text-center">
            <Zap className="mx-auto mb-4 h-14 w-14 animate-pulse text-pa-green" />
            ...
            <button
              onClick={handleStart}
              disabled={starting}
              className="mt-8 rounded-xl bg-pa-green px-10 py-4 text-lg font-bold text-black transition-all hover:bg-pa-green/80 hover:scale-105 disabled:opacity-50"
            >
              {starting ? (
      ...
      {phase === "quiz" && question && (
        <>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <div className="h-2 overflow-hidden rounded-full bg-surface-alt">
                <div
                  className="h-full rounded-full bg-pa-green transition-all duration-500"
                  style={{
                    width: `${((participant?.answeredCount ?? 0) / (participant?.totalQuestions ?? 15)) * 100}%`,
                  }}
                />
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-border bg-surface p-6 md:p-8">
            ...
            <h2 className="mt-4 text-xl font-bold leading-tight text-text-primary md:text-2xl">
              {question.question}
            </h2>
            ...
            <div className="mt-6 space-y-3">
              {question.answers.map((answer, idx) => {
                ...
                if (lastResult) {
                  ...
                  btnClass +=
                    "border-border bg-surface-alt text-text-primary hover:border-pa-green/50 hover:bg-pa-green/5";
                }
                ...
                return (
                  <button
                    ...
                    className={btnClass}
                  >
                    <span className="mr-3 inline-flex h-7 w-7 items-center justify-center rounded-full border border-current text-sm font-bold">
      ...
      {phase === "completed" && (
        <>
          {/* Results card */}
          <div className="rounded-2xl border border-border bg-surface p-8 text-center">
            <Trophy className="mx-auto mb-4 h-14 w-14 text-yellow-400" />
            <h1 className="text-2xl font-bold text-text-primary">
              Quiz abgeschlossen!
            </h1>
            ...
            <div className="mt-6 flex items-center justify-center gap-8">
              <div>
                <p className="text-3xl font-bold text-pa-green">
                  {participant?.correctCount ?? 0}
                  <span className="text-lg text-text-muted">
                    /{participant?.totalQuestions ?? 15}
                  </span>
                </p>
                <p className="text-xs text-text-muted">Richtige Antworten</p>
              </div>
              <div>
                ...
                <p className="text-xs text-text-muted">Quote</p>
              </div>
              <div>
                <p className="text-3xl font-bold text-text-primary">
                  {formatTime(participant?.totalTimeMs ?? 0)}
                </p>
                <p className="text-xs text-text-muted">Gesamtzeit</p>
              </div>
            </div>
