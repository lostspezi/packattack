...
export class OverlayLayer extends Container {
  ...
  resize(w: number, h: number): void {
    ...
    this.mainText.x = w / 2;
    this.mainText.y = h / 2;
    // Scale text instead of changing fontSize
    const mainScale = (h * ROUND_ANNOUNCE.numberFontSize) / 80;
    this.mainText.scale.set(mainScale);

    this.subText.x = w / 2;
    this.subText.y = h / 2 - h * 0.1;
    const subScale = (h * ROUND_ANNOUNCE.labelFontSize) / 18;
    this.subText.scale.set(subScale);
  }
