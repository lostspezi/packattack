...
export function ShippingTiersManager({ dict }: { lang: string; dict: Record<string, string> }) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const data = await res.json();
      if (data._id) {
        toast({ type: "success", title: dict["shipping_tierCreated"] ?? "Stufe erstellt" });
        setShowForm(false);
        fetchTiers();
      } else {
    ...
    <div className="space-y-4">
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>
            {tiers.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-text-muted">{dict["shipping_noTiers"] ?? "Keine Stufen"}</td></tr>
            ) : tiers.map((t) => (
      ...
      {!showForm ? (
        ...
        <div className="rounded-lg border border-border bg-card p-4 space-y-3">
          <h3 className="text-sm font-semibold text-text-primary">{dict["shipping_newTier"] ?? "Neue Stufe"}</h3>
