...
type CardDecision = "claimed" | "converted";
...
export function PackOpening({ result, box, lang, onDone, onCoinsChange }: PackOpeningProps) {
  ...
  async function handleDecision(decision: "claim" | "convert") {
    ...
    try {
      const res = await fetch("/api/pulls/decide", {
        ...
        body: JSON.stringify({
          ...
          coinValue: currentCard.coinValue,
          conversionValue: currentCard.conversionValue,
          decision,
          boxId: box._id,
        }),
      });
      ...
      setDecisions((prev) => new Map(prev).set(currentIndex, decision === "claim" ? "claimed" : "converted"));
  ...
  if (finished) {
    ...
    const coinsReceived = cards.reduce((sum, c, i) => {
      return decisions.get(i) === "converted" ? sum + c.conversionValue : sum;
    }, 0);
      ...
      <div className="max-w-lg mx-auto space-y-6 py-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-green-500/5 border border-green-500/15 rounded-xl p-4 text-center">
            <p className="text-3xl font-bold text-green-400">{claimed}</p>
            <p className="text-xs text-text-muted mt-1">{isDe ? "Geclaimed" : "Claimed"}</p>
          </div>
          <div className="bg-blue-500/5 border border-blue-500/15 rounded-xl p-4 text-center">
            <p className="text-3xl font-bold text-blue-400">{converted}</p>
            <p className="text-xs text-text-muted mt-1">
              {isDe ? "Umgewandelt" : "Converted"}
              {coinsReceived > 0 && <span className="text-pa-green ml-1">(+{coinsReceived} Coins)</span>}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {cards.map((c, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3 bg-white/3 border border-border rounded-xl p-2.5">
                {c.image ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={c.image} alt="" className="w-10 rounded shrink-0" loading="lazy" />
                ) : (
                  <div className="w-10 aspect-[63/88] bg-white/4 rounded shrink-0" />
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-text-primary truncate">{c.name}</p>
                  <Badge variant="info">{c.rarity}</Badge>
                </div>
                <Badge variant={d === "claimed" ? "success" : "info"}>
                  {d === "claimed" ? (isDe ? "Geclaimed" : "Claimed") : `+${c.conversionValue} Coins`}
                </Badge>
              </div>
            );
          })}
