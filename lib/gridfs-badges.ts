...
/**
 * Normalisiert ein hochgeladenes Badge-Icon auf 256x256 WebP. Quadratischer
 * contain-Zuschnitt mit transparentem Hintergrund — nicht-quadratische
 * Vorlagen (z.B. breite Wappen) bleiben so unverzerrt sichtbar.
 */
export async function uploadBadgeIcon(
  ...
  const id = await new Promise<ObjectId>((resolve, reject) => {
    const upload = bucket.openUploadStream(`badge-${Date.now()}.webp`, {
...
export async function getBadgeIcon(
  id: ObjectId,
...
export async function deleteBadgeIcon(id: ObjectId): Promise<void> {
  const bucket = await getBucket();
  try {
