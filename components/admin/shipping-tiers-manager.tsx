...
export function ShippingTiersManager({ lang, dict }: { lang: string; dict: Record<string, string> }) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const data = await res.json();
      if (data._id) {
        toast({ type: "success", title: isDe ? "Staffel erstellt" : "Tier created" });
        setShowForm(false);
        fetchTiers();
      } else {
    ...
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-white/2">
              <th className="px-4 py-3 text-left text-xs font-medium text-text-muted">{isDe ? "Land" : "Country"}</th>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-text-muted">{isDe ? "Preis (Cent)" : "Cost (Cents)"}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-text-muted">Coins</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-text-muted">{isDe ? "Aktiv" : "Active"}</th>
            </tr>
          </thead>
          <tbody>
            {tiers.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-text-muted">{isDe ? "Keine Staffeln" : "No tiers"}</td></tr>
            ) : tiers.map((t) => (
      ...
      {!showForm ? (
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 rounded-lg bg-pa-green/10 px-4 py-2 text-sm font-medium text-pa-green hover:bg-pa-green/20">
          <Plus className="h-4 w-4" /> {isDe ? "Staffel hinzuf\u00fcgen" : "Add Tier"}
        </button>
      ) : (
        <div className="rounded-lg border border-border bg-card p-4 space-y-3">
          <h3 className="text-sm font-semibold text-text-primary">{isDe ? "Neue Staffel" : "New Tier"}</h3>
